...
function createSpectrogram(buffer, fftSize = 1024, hopSize = 256, melCount=96, returnsImage = false){
    const channelOne = buffer.channelData[0];  // use only the first channel
    const sampleRate = buffer.sampleRate;
    const db_spectrogram = [];
 
    // Create a fft object. Here we use default "Hanning" window function
    const fft = new RFFT(fftSize, sampleRate); 
